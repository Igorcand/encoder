...
func (j *JobManager) Start(ch *amqp.Channel){
	...
	for qntProcess := 0; qntProcess < concurrency; qntProcess++{
		go JobWorker(j.MessageChannel, j.JobReturnChannel, jobService, j.Domain, qntProcess)
	}
...
func (j *JobManager) checkParseErrors(jobResult JobWorkerResult) error{
	if jobResult.Job.ID != ""{
		log.Println("MessageID #{jobResult.Message.DeliveryTag} , Error parsing job: #{jobResult.Job.ID}")
	} else{
		log.Println("MessageID #{jobResult.Message.DeliveryTag} , Error parsing message: #{jobResult.Error}")
	}
